# Adding Gald3r to an Existing Project (OpenHands)

Add gald3r to a project that **already exists**, for use with **OpenHands**.

For a brand-new project, see [instructions_new_project.md](./instructions_new_project.md).

---

## Step 1 - Get the `openhands/` payload

```bash
git clone --depth 1 https://github.com/wrm3/gald3r_platform_openhands.git
```
...
**bash / macOS / Linux:**
```bash
cp -r gald3r_platform_openhands/openhands/. /path/to/your/existing/project/
```

**Windows PowerShell:**
```powershell
Copy-Item -Recurse -Force .\gald3r_platform_openhands\openhands\* C:\path\to\your\project\
```
...
Reload the project in OpenHands. gald3r auto-loads from its config.
